...
namespace Manager
{
    public class UniRxManager : SingletonMonoBehaviour<UniRxManager>
    {
        ...
        //カメラ方向変更マウスイベントを発行する核となるインスタンス
        private Subject<bool> changeCameraDirMouseSubject = new Subject<bool>();

        /// <summary>
        /// カメラ方向変更マウスイベントの購読側だけを公開
        /// </summary>
        public IObservable<bool> OnChangeCameraDirMouseEvent
        {
            get { return changeCameraDirMouseSubject; }
        }

        /// <summary>
        /// カメラ方向変更マウスイベント発行
        /// </summary>
        /// <param name="mousedown">マウスダウン</param>
        public void SendChangeCameraDirMouseEvent(bool mousedown)
        {
            changeCameraDirMouseSubject.OnNext(mousedown);
        }
    }
}
